            ...
            <tbody class="divide-y divide-[#d4af37]/10 bg-[#0e1526] text-[#f0e1be]">
                @foreach ($rows as $index => $row)
                    <tr>
                        ...
                        <td class="px-4 py-3">
                            <input wire:model.live="rows.{{ $index }}.slug" type="text" class="w-full rounded border border-[#d4af37]/30 bg-[#111b31] px-2 py-1 text-sm text-[#f5e7c4] focus:border-[#d4af37]/65 focus:outline-none">
                        </td>
                        <td class="px-4 py-3">
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-2">
                                ...
                                <button type="button" wire:click="revisionHistory({{ $index }})" title="Revision History" class="rounded border border-[#d4af37]/30 p-1.5 text-[#d9c594] hover:border-[#d4af37]/60"><svg class="size-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M3 12a9 9 0 1 0 3-6.7"/><path d="M3 4v4h4"/><path d="M12 7v5l3 2"/></svg></button>
                            </div>
                        </td>
                    </tr>
                @endforeach
